...
# run instances with list by AriParti
...
output_dir_pre_path_template = r'AE-test-output/{}/raw-results/{}/AriParti-{}-p{}'
...
def solve(job : str, cnt : int):
    ...
    output_dir_path = output_dir_pre_path + job[ : job.find('.smt2')]
    try:
        cmd = ' '.join(['python3', script_path,
                        '--file', input_path,
                        '--output-dir', output_dir_path,
                        '--partitioner', partitioner_path,
                        '--solver', base_solver_path,
                        '--max-running-tasks', str(parallel_core_num),
                        '--time-limit', f'{time_limit:.0f}',
                    ])
    ...
    cost_time : float = float(result_lines[1])
...
def RIWL_AriParti(_parallel_core_num: int, _max_process_num: int, 
                  _benchmark_tag: str, _list_tag: str, _base_solver_tag: str, 
                  _test_type: str, _lists_dir_path: str, _time_limit: float):
    
    init_counters()
    
    global parallel_core_num
    ...
    parallel_core_num = _parallel_core_num
    ...
    output_dir_pre_path = output_dir_pre_path_template.format(test_type, list_tag, _base_solver_tag, parallel_core_num)
    ...
    print(f'total job number: {total_job}')
...
if __name__ == '__main__':
    ...
    RIWL_AriParti(8, 24, 'QF_LRA-1753', 'cvc5-1.0.8', 'all', 'benchmark-lists/all', 60)
